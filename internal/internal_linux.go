...
func InitProcess() {
	...
	if os.Getenv("EMBEDDED_QTBOX") == "true" {

		runPath = "qtbox"
		runPathZipped := runPath + ".zip"

		if st, err := os.Stat(runPath); err != nil || st.Size() == 0 {

			// Извлекаем бинарник в временный файл
			zippedQtbox, err := resources.ReadFile("resources/" + runPathZipped)
			if err != nil {
				log.Fatal(err)
			}

			// Создаем reader для архива из куска памяти
			r, err := zip.NewReader(bytes.NewReader(zippedQtbox), int64(len(zippedQtbox)))
			if err != nil {
				log.Fatal(err)
			}
			...
			rc, err := f.Open()
			if err != nil {
				fmt.Printf("Ошибка открытия файла из архива: %v\n", err)
				outFile.Close()
				log.Fatal(err)
			}
			...
			if err != nil {
				fmt.Printf("Ошибка копирования данных: %v\n", err)
				log.Fatal(err)
			}
