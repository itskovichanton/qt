...
func Rcc(path, target, tagsCustom, output_dir string, useuic, quickcompiler, deploying bool, skipSetup bool) {
	if utils.UseGOMOD(path) && !skipSetup {
		...
		if utils.QT_DOCKER() {
			cmd := exec.Command("go", "get", "-v", "-d", "github.com/itskovichanton/qt/internal/binding/files/docs/"+utils.QT_API(utils.QT_VERSION())+"@"+cmd.QtModVersion(filepath.Dir(utils.GOMOD(path)))) //TODO: needs to pull 5.8.0 if QT_WEBKIT
			cmd.Dir = path
			...
			for _, filePath := range fileList {
				...
				for i, p := range []string{"runtime", "core", "gui", "widgets", "webengine", "multimedia", "uitools", "quick"} {
					...
					if i == 0 {
						...
						fmt.Fprintln(bb, "\"github.com/itskovichanton/qt/"+p+"\"")
					}
				}
				fmt.Fprintln(bb, "\n)")
