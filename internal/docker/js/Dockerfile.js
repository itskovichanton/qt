...
RUN /usr/local/go/bin/go get -tags=no_env github.com/itskovichanton/qt/cmd/...
...
COPY --from=base $GOPATH/src/github.com/itskovichanton/qt $GOPATH/src/github.com/itskovichanton/qt
COPY --from=itskovichanton/qt:js_base $HOME/emsdk $HOME/emsdk
COPY --from=itskovichanton/qt:js_base $HOME/.emscripten $HOME/.emscripten
COPY --from=itskovichanton/qt:js_base /usr/local/Qt-5.13.0 /usr/local/Qt-5.13.0
...
RUN cd $GOPATH/src/github.com/itskovichanton/qt/internal/examples/widgets/line_edits && $GOPATH/bin/qtdeploy build js && rm -rf ./deploy
